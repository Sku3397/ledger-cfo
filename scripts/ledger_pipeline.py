...
def main():
    ...
    parser.add_argument(
        "--start-from", type=int, default=0, help="Step to start from (0-based)"
    )
    parser.add_argument(
        "--end-at", type=int, default=None, help="Step to end at (0-based, inclusive)"
    )
    parser.add_argument(
        "--list", action="store_true", help="List the pipeline steps and exit"
    )
    ...
    if args.end_at is not None and (
        args.end_at < args.start_from or args.end_at >= len(PIPELINE_STEPS)
    ):
        print(
            f"Error: end-at must be between {args.start_from} and {len(PIPELINE_STEPS) - 1}"
        )
        return
