...
class MypyDaemonManager:
    ...
    def run_check_with_remote_cache(self, branch="main", repo_url=None, files_or_dirs=None, run_flags=None):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if download_result.returncode != 0:
                print(f"Failed to download cache data: {download_result.stderr.strip()}")
                return self.run_check_incremental(files_or_dirs, run_flags)
            ...
            # Update mypy flags
            self.mypy_flags = [flag for flag in self.mypy_flags if not flag.startswith("--cache-dir=")]
            self.mypy_flags.append(f"--cache-dir={self.cache_dir}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Manage the mypy daemon for improved type checking performance')
    parser.add_argument('command', choices=['start', 'stop', 'restart', 'status', 'run', 'run-cached'],
                       help='Command to run')
    ...
    parser.add_argument('--branch', default='main', help='Branch to use as base for remote cache (for run-cached)')
    parser.add_argument('--repo-url', help='Repository URL containing cache data (for run-cached)')
    
    args = parser.parse_args()
    
    manager = MypyDaemonManager(
        mypy_flags=args.mypy_flags,
