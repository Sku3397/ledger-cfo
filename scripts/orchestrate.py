...
def run_tests(args):
    ...
    cmd = [
        "python", "scripts/terminal_orchestrator.py",
        "--name", "ledger-tests",
        "--cmd", args.test_command or "pytest"
    ]
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Orchestrate the entire ledger process')
    ...
    parser.add_argument('--check-files', nargs='*', help='Files or directories to type check')
    
    # Test options
    parser.add_argument('--test-command', help='Command to run tests (default: pytest)')
    
    # Deployment options
    parser.add_argument('--start-from', type=int, help='Step to start deployment from (0-based)')
    parser.add_argument('--end-at', type=int, help='Step to end deployment at (0-based, inclusive)')
