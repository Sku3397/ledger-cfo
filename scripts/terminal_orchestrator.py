    ...
    def _poll_process(self):
        ...
        for interval in self.poll_intervals:
            ...
            if self.process.poll() is not None:
                ...
                print(
                    f"Command '{self.name}' completed with exit code {self.exit_code}"
                )
                break
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run a command with polling and timeout"
    )
    ...
    parser.add_argument(
        "--log-dir", default="scripts/logs", help="Directory to store logs"
    )

    args = parser.parse_args()

    orchestrator = TerminalOrchestrator(
        name=args.name, cmd=args.cmd, log_dir=args.log_dir
    )
